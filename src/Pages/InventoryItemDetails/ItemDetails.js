...
const ItemDetails = () => {
    ...
    useEffect(() => {
        const loadProduct = async () => {
            const url = `https://unique-shop-server-production.up.railway.app/warehouseproducts/${itemId}`
    ...
    const handleUpdateQuantity = async ({ updatingQuantity }) => {
        setSnackBar(true)
        if (updatingQuantity) {
            const response = await fetch(`https://unique-shop-server-production.up.railway.app/warehouseproducts/${itemId}`, {
                method: "PUT",
                headers: {
